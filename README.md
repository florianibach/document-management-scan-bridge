...
Compose runs the application as `${APP_UID:-1000}:${APP_GID:-1000}`. Set these variables to the account that should own the host files when its IDs differ from `1000`, for example `APP_UID=$(id -u) APP_GID=$(id -g) docker compose up --detach --build`. Merely adding `APP_UID` to a service's `environment` does **not** change the process user selected by the image's `USER` instruction; Compose's `user` setting does. Likewise, `user: 1000:1000` alone cannot make a root-owned bind mount writable.

When a bind-mount source is missing, Docker creates it as `root`. The one-shot `scan-bridge-permissions` service therefore changes `./app/data` and `./app/temp` recursively to the configured IDs before Compose starts the unprivileged `scan-bridge` service. The ownership of the parent `./app` directory is not the problem as long as it is traversable; the mounted `data` and `temp` directories must be writable. `/app/data/dataprotection-keys` is a directory rather than a pre-existing file, and the application creates it during startup once `/app/data` is writable. Keep the permissions service when changing the bind-mount paths, and start the complete stack through Compose rather than running only the application container.
