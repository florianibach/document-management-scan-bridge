...
| `PROFILE_ALLOW_DEPLOYMENT_TOKEN_FALLBACK` | `ProfileServices__AllowDeploymentTokenFallback` | `false` | Allows authenticated profiles without their own token to use the shared administrator-managed `PAPERLESS_TOKEN`. Securely disabled by default; not secret and cannot be changed by profile owners. |
...
The active Send step keeps metadata and upload failures visible until they are retried, succeed, are deliberately dismissed, or the workflow is left. It distinguishes missing configuration, invalid authentication (HTTP 401), missing permission (HTTP 403), network and timeout problems, invalid responses, and Paperless server failures. Configuration and permission failures link directly to Paperless settings; all failures offer the relevant retry. A short diagnostic ID can be copied into an operator report and correlated with `docker compose logs` without exposing tokens, response bodies, document contents, metadata, or filenames. For split batches, the alert names the affected workflow document, and a failed document remains unsent and retryable while already accepted documents are never uploaded again. `PAPERLESS_TIMEOUT_SECONDS` changes the default 60-second limit when needed. Paperless performs OCR and its normal processing; the bridge does not wait for completion.
...
Sobald sich ein editierbarer Profilwert ändert, bleibt oben im Profilbereich ein Hinweis auf ungespeicherte Änderungen sichtbar. **Alle Profileinstellungen speichern** übernimmt Paperless-Verbindung, Dokumentvorgaben und Scannerwerte gemeinsam und zeigt genau ein Ergebnis neben dieser Aktion. Dabei wird die URL syntaktisch und sicherheitsbezogen geprüft, die Speicherung aber nicht von der momentanen Erreichbarkeit der Paperless-Instanz abhängig gemacht. **Verbindung testen und Dokumentauswahl laden** bleibt als bewusster Online-Test verfügbar. Paperless-Dokumentvorgaben stehen zusammen vor den Scannerwerten; Tags lassen sich platzsparend als Mehrfachauswahl wählen. Bei Navigation innerhalb der Anwendung sowie beim Neuladen oder Schließen des Tabs bittet der Browser vor dem Verwerfen ungespeicherter Profiländerungen um Bestätigung.

Vor dem Speichern werden Scannerquelle und Auflösung gegen die gespeicherten Fähigkeiten geprüft. Über **Gespeicherte Verbindung testen und Dokumentauswahl laden** werden gespeicherte Metadaten-IDs gegen die aktuelle Paperless-Instanz geprüft. Entfernte oder nicht mehr unterstützte Werte werden angezeigt und müssen korrigiert werden. **Dokument- und Scanner-Vorgaben zurücksetzen** entfernt den lokalen Vorgabedatensatz vollständig.
...
The settings page can validate and activate a profile-specific Paperless URL and API token. The encrypted profile token is loaded only for its authenticated owner into a password input with an explicit Show/Hide control. Anyone who can use the owner’s active signed-in browser session can therefore reveal it. Keep sessions and devices locked; the token is never placed in URLs, logs, diagnostics, browser storage, or a normal text input.

If an authenticated profile has no saved URL, `PAPERLESS_URL` prepopulates the form. It can be saved as that profile’s URL when overrides are enabled; otherwise it stays read-only and administrator-managed. An authenticated profile token always has priority for checks, metadata, and uploads. Only when `PROFILE_ALLOW_DEPLOYMENT_TOKEN_FALLBACK=true` may a profile without one use the shared administrator-managed `PAPERLESS_TOKEN`; profile owners cannot enable that policy. All such uploads use the same Paperless account, whose permissions determine assignment and visibility. With fallback disabled (the default), a configured deployment token is reserved for anonymous mode and startup emits a secret-free warning without failing health checks. Anonymous mode continues to use the deployment URL and token without delivering the token to the browser.
...
Profile API tokens are encrypted with ASP.NET Core Data Protection before SQLite writes. The plaintext is returned only to the authenticated owner’s protected settings session and remains in a password input unless Show is explicitly selected; use the explicit save/check and remove controls for rotation. Deployment tokens are read at process startup and never copied into SQLite. Back up profile-token rows together with the Data Protection key ring; mode changes do not copy or merge tokens, and authenticated fallback policy never changes anonymous behavior. Scan-session download routes have a persisted profile owner and return not-found across profile boundaries; metadata/default records and service configuration are keyed by the same internal profile ID. Do not place sensitive values in logs.
