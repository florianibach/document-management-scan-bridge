...
The application runs as the image's unprivileged `APP_UID`. Docker can create missing bind-mount source directories as `root`, so the one-shot `scan-bridge-permissions` service assigns both directories to `APP_UID` before Compose starts `scan-bridge`. This also explains why `dataprotection-keys` does not need to exist beforehand: the application creates that subdirectory itself once its parent is writable. Keep the permissions service when changing the bind-mount paths, and start the stack through Compose rather than running only the application container.
