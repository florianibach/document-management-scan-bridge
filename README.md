...
| `PROFILE_SIGNOUT_MODE` | `Profiles__SignOutMode` | `ProviderWithLocalFallback` | Logout behavior. `ProviderWithLocalFallback` clears the local cookie and then uses `PROFILE_REMOTE_SIGNOUT_URL` when set, otherwise the provider metadata logout flow; failures fall back to local completion. `LocalOnly` never contacts the provider. |
| `PROFILE_REMOTE_SIGNOUT_URL` | `Profiles__RemoteSignOutUrl` | empty | Optional absolute HTTPS logout URL used after local cookie removal. Useful for providers such as Google that do not advertise a standard OIDC `end_session_endpoint`; for Google this can be `https://accounts.google.com/Logout`. |
...
4. Set `PROFILE_MODE=OpenIdConnect`, `OIDC_AUTHORITY=https://accounts.google.com`, and serve Scan Bridge through HTTPS. Google does not currently advertise a standard OIDC `end_session_endpoint` in its discovery document; if users should also leave their Google account session, set `PROFILE_REMOTE_SIGNOUT_URL=https://accounts.google.com/Logout`. Keep forwarded headers enabled at the reverse proxy so generated redirect URIs use the external HTTPS origin.
