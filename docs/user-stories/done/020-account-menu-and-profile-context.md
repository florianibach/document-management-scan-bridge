...
- Anonymous mode identifies the shared household profile and explains that its settings are shared. Authenticated mode shows only the resolved display name, a provider hostname or generic OpenID Connect context, an HTTPS profile image when safely usable (otherwise initials), and the existing local sign-out action.
