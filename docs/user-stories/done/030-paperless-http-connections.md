...
- Whenever the configured or effective Paperless URL uses HTTP, the relevant Paperless settings UI clearly states that the connection is unencrypted and that credentials, metadata, and documents can be observed or changed in transit. HTTPS remains the clearly recommended option. A typed deployment option may suppress only this notice, defaults to showing it, and does not change URL handling or transport security.
...
- Compose exposes `PAPERLESS_URL` and the default-on `PAPERLESS_SHOW_HTTP_WARNING`; the default URL remains valid HTTP. README configuration, warning suppression, validation, risk, HTTPS recommendation, recreation, and Compose-validation guidance covers both supported schemes and makes clear that suppression changes presentation only.
