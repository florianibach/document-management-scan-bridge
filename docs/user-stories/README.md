...
| [US-025](done/025-paperless-processing-hint-after-upload.md) | Paperless processing hint after upload (done) | Accepted uploads clearly explain that Paperless processing may continue |
| [US-026](done/026-notify-scan-timeout-decision.md) | Notify scan timeout decision (done) | Background users are called back when an active scan needs a timeout decision |
...
| [US-030](030-paperless-http-connections.md) | Paperless HTTP connections | Local Paperless deployments can use explicitly configured HTTP with clear security guidance |
